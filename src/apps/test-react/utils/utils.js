...
export const getRadian = (startSprite, endSprite) => {
  const endX = endSprite.x + endSprite.width / 2;
  const endY = endSprite.y + endSprite.height / 2;
  const startX = startSprite.x + startSprite.width / 2;
  const startY = startSprite.y + startSprite.height / 2;
  const dx = endX - startX;
  const dy = endY - startY;
  const dl = Math.sqrt(Math.pow(dx, 2) + Math.pow(dy, 2));
  const cos = dx / dl;
  const radian = dy < 0 ? 2 * Math.PI - Math.acos(cos) : Math.acos(cos);
  return radian;
};
