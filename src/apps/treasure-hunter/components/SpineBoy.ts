...
export default class SpineBoy extends PIXI.spine.Spine {
    constructor(spineData: PIXI.spine.core.SkeletonData) {
        ...
        emitter.on(Events.STATE_CHANGE, (state: ScreenState, oldState: ScreenState) => { this.parent && this.onStateChange(state, oldState) });
    }
    ...
    private initSpineBoy() {
        this.scale.set(0.3);
        this.position.set(resolution.size.width / 2, resolution.size.height - 75 * this.scaleValue);
        this.interactive = true;
        this.buttonMode = true;
        this.shadowFilter = new PIXI.Filter(`
        ...
        context.app.stage.interactive = true;
        context.app.stage.on('pointerdown', ()=>{
    ...
    private containCheck() {
        ...
        if (this.x > resolution.size.width - this.width / 2) this.x = resolution.size.width - this.width / 2;
    }

    private ticker() {
        this.x += this.vx;
        this.y += this.vy;
        this.shadowFilter.uniforms.floorY = this.toGlobal(new PIXI.Point(0, 0)).y;
        this.containCheck();
    }

    public onStateChange(state: ScreenState, oldState: ScreenState) {
        if (!oldState ||
            (state == ScreenState.LANDSCAPE && oldState == ScreenState.WEB) ||
            (state == ScreenState.WEB && oldState == ScreenState.LANDSCAPE)
        ) return;

        this.position.set(this.x * resolution.size.ratio, resolution.size.height - 75 * this.scaleValue);
    }
