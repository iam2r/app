...
export default class ParallaxScroller extends Container {
    ...
    private initParallaxScroller() {
        Array.from({ length: 2 }).forEach(() => {
            let background = new Sprite(Resources.scrollerBg.textures['bgtile.jpg']);
            background.scale.set(this.scaleValue);
            let foreground = new Sprite(Resources.scrollerBg.textures['ground.png']);
    ...
    private setParallaxScroller(sprites: Sprite[], postition: number = this.postition) {
        let width = sprites[0].width;
        let [first, clone] = sprites;
        first.x = -postition;
        first.x %= width * 2;
        if (first.x < 0) {
            first.x += width * 2;
