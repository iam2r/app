...
export default class Utils {
    ...
    static formatAmount(value: number, fractionDigets: number = 2, currency?: string) {
        currency = currency ? currency : "";
        value = Number.parseFloat(value.toFixed(6))
        value = Math.floor(value * 100) / 100;
        let str = value.toFixed(fractionDigets);
        let arr = str.split("."), p = arr[0], temp = [];
        arr[1] = arr[1] || "";
        while (p.length > 2) {
            let len = p.length - 3;
    ...
    static toLocalTimeByTimeZone(time: number, zoneNum: number) {
        ...
        let offset = Math.floor(new Date().getTimezoneOffset() * 60000);
