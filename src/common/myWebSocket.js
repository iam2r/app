...
class Service {
  ...
  connect() {
    if (!this.regUrl.test(this.url))
      return this._printLog("websocket: url error!");
  ...
  send(command, data, callback) {
    if (!this.socket || this.socket.readyState == WebSocket.CLOSED)
      this.connect(); //不先调用connect 直接send时的处理
  ...
  _createWebSocket(resolve, reject) {
    ...
    this.socket.onopen = async (e) => {
      ...
      const { salt, key, iv } = await this.send(this.CRYPTO_AES, {}, null);
      this._initAES(salt, key, iv);
      resolve(this.socket);
      Object.values(this.promisePool).forEach(({ plainText }) => {
    ...
    this.socket.onmessage = (e) => {
      ...
      cipherText = e.data;
      plainText =
        e.data.split(".")[0] == this.CRYPTO_AES
          ? cipherText
          : this._decryption(cipherText);
      ...
      const key =
        response.command < 0 || response.command == this.CRYPTO_AES
          ? response.command
  ...
  _parseReqData(data) {
    try {
      if (this.regMessage.test(data)) {
        ...
        const dataArr = array[1].split("|");
        const key = dataArr[0].trim(),
          iv = dataArr[1].trim(),
          salt = dataArr[2].trim();
        return {
          command,
