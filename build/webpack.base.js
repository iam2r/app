...
process.env.APP_ROOT = path.resolve(
  __dirname,
  `../src/apps/${process.env.APP_NAME}`
);

const { plugins: SpritesmithPlugin } = require("./spritesmith.config");

const doResourceCache = (obj) => {
  !~resourceCache.findIndex(
    ({ resourcePath }) => resourcePath == obj.resourcePath
  ) && resourceCache.push(obj);
};

const createEntryAndHtml = (pageList) =>
  pageList.reduce(
    (pre, page) => {
...
const { entry, html } = createEntryAndHtml(
  utils.getFileNameList("./public", "html")
);
const context = path.resolve(__dirname, "../");
process.env.ENTRY_FILES = JSON.stringify(
  Object.values(entry || [])
    .reduce((allEntries, curr) => {
...
const plugins = [
  ...
  new ProgressBarPlugin({
    ...
    )}${chalk.bold("[")}:bar${chalk.bold("]")}${chalk.green.bold(
      ":percent"
    )}(:elapsed seconds)`,
  }),
  new ProvidePlugin({
...
const copyWebpackOptions = [
  ...
  },
  {
    from: path.resolve(
      __dirname,
      `../src/apps/${process.env.APP_NAME}/_public`
    ),
    to: config.staticPath + "/" + process.env.APP_NAME,
    toType: "dir",
    force: true,
...
plugins.push(
  new CopyWebpackPlugin({
    patterns: copyWebpackOptions.filter(({ from }) => fs.existsSync(from)),
  })
);
...
module.exports = {
  ...
  output: {
    ...
    filename: utils.isDev()
      ? "js/[name].js"
      : `js/[name]${
          process.env.BUILD_MODE == BUILDMODE.legacy
            ? `-${BUILDMODE.legacy}`
            : ""
        }.[contenthash:8].js`,
          ...
          },
          {
            use: [
              {
                loader: "cache-loader",
                options: {
                  cacheIdentifier: utils.isDev()
                    ? "dev"
                    : process.env.BUILD_MODE == BUILDMODE.modern
                    ? BUILDMODE.modern
                    : BUILDMODE.legacy,
            ...
            exclude: [
              (filepath) => {
                ...
                if (
                  config.transpileDepRegex &&
                  config.transpileDepRegex.test(filepath)
                ) {
                  return false;
                }
